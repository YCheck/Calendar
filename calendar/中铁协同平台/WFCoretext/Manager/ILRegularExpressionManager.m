...
+ (NSArray *)itemIndexesWithPattern:(NSString *)pattern inString:(NSString *)findingString
{
    ...
    if (ranges.count > 0) {
        ...
        NSMutableArray *returnRangs = [NSMutableArray arrayWithArray:ranges];
        [self returnArr:findingString itemIndexes:&returnRangs];
        return returnRangs;
    }
    return ranges;
}

+ (void)returnArr:(NSString *)findingString itemIndexes:(__strong NSMutableArray **)itemIndexes
{
    ...
    for (int i=0; i<(*itemIndexes).count; i++) {
        ...
        NSString *name = [findingString substringWithRange:[(*itemIndexes)[i] rangeValue]];
        ...
        if (faceImage == nil) {
            [(*itemIndexes) removeObjectAtIndex:i];
            i--;
        }
    }
}
