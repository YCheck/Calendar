...
//#define FontHeight                  15.67
#define ImageLeftPadding            0.0
#define ImageTopPadding             6.0
//#define FontSize                    FontHeight
//#define LineSpacing                 10.0
#define EmotionImageWidth           22
...
- (void)setOldString:(NSString *)oldString andNewString:(NSString *)newString fromSelf:(BOOL)fromSelf{
    
    _oldString = oldString;
    _newString = newString;
    [self cookEmotionStringfromSelf:fromSelf];
}

#pragma mark - Cook the emotion string
- (void)cookEmotionStringfromSelf:(BOOL)fromSelf
{
    ...
    names = [_oldString itemsForPattern:EmotionItemPattern captureGroupIndex:0];
    ...
    _attrEmotionString = [self createAttributedEmotionStringWithRanges:newRanges
                                                             forString:_newString fromSelf:fromSelf];
    ...
    float width1 = [self sizeWithString:_newString font:_font width:320].width;
    float width2 = [self sizeWithString:PlaceHolder font:_font width:320].width;
    float width3 = width1 - width2 * names.count + names.count * EmotionImageWidth;
    if (width3 < self.frame.size.width) {
        self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, width3, self.frame.size.height);
    }
                                                       ...
                                                       fromSelf:(BOOL)fromself
{
    ...
    helvetica = CTFontCreateWithName((CFStringRef)_font.fontName,
                                     _font.pointSize,
                                     NULL);
    ...
    [attrString addAttribute:(id)kCTForegroundColorAttributeName value:(id)(fromself?[UIColor whiteColor].CGColor:HEXCOLOR(0x333333).CGColor) range:NSMakeRange(0,[attrString length])];
...
// 通过表情名获得表情的图片
- (UIImage *)getEmotionForKey:(NSString *)str
{
...
{
    UIFont *font = [UIFont systemFontOfSize:15.7];
...
- (void)drawRect:(CGRect)rect
{
    ...
    // 翻转坐标系
    Flip_Context(context, _font.pointSize);
    ...
    while (start < length)
    {
        ...
        start += count;
        y -= _font.pointSize + _lineSpacing;
        CFRelease(line);
